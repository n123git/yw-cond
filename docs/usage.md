...
* Comments `;` - these can be used to define single-line comments.
* Brackets `()` - these can be used to define explicit operator precedence
* Integer Literals `3` - (signed 32-bit integer) these can be defined as `<literalNumber>` i.e. `1`, `0`, `-3`, `-10000` etc
* Hex Literals `0x1234578` (signed 32-bit integer) these can be defined using `0x` notation or with the `h`/`u` keywords i.e. `0x1234`, `2h`, `10u`, `0x1234h`, `0x12345678u`.
* Char Literals `'a'` - these are an alias for Hex Literals and must contain no more than one character, which must be no more than one byte.
