...
* Operators - expects an [infix expression](http://en.wikipedia.org/wiki/Infix_notation) i.e. `3 + 2` not `3 2 + `. Additionally note that unary operators must be placed before the operand i.e. `++3`. Check the [[format specification]] for a list of operators.
